    ...
    <Screen noKeyboardAwareScroll backgroundColor="white">
      ...
      >
        <View className="px-5 pb-10">
          {rides
            .filter((activity: ActivityItem) => activity.status === serviceType)
            .map((activity) => (
...
function ActivityContent({ activity }: { activity: ActivityItem }) {
  ...
  return (
    <View
      key={activity.id}
      className="my-2 border-primary rounded-xl px-3 pt-1 pb-2"
      style={{ borderWidth: 1 }}
    >
      ...
      <View>
        <View className="flex flex-row items-center pb-2">
          <MaterialIcons color={colors.primary} name="my-location" size={25} />
          <Text className="px-2 text-base">{activity.pickLocation}</Text>
        </View>
        <View className="flex flex-row items-center ">
          <MaterialIcons color={colors.primary} name="location-on" size={25} />
